...
func GetTagList(db *gorm.DB, page, size int, keyword string) (list []TagV0, total int64, err error) {
	db = db.Table("tag t").
...
func GetTagNamesByArticleId(db *gorm.DB, id int) ([]string, error) {
	list := make([]string, 0)
	result := db.Table("tag").Joins("LEFT JOIN article_tag ON tag.Id = article_tag.tag_id").
		Where("article_id", id).
		Pluck("name", &list)
	return list, result.Error
}
