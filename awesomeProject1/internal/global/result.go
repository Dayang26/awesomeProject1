...
var (
	...
	_message = make(map[int]string) //依据错误码获取错误信息
)

// 注册一个响应码,don't allowed hava repeat registed
...
func GetMsg(code int) string { return _message[code] }

var (
	OkResult   = RegisterResult(SUCCESS, "OK")
	FailResult = RegisterResult(Failed, "FAIL")
)
